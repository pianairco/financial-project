...
public class GenericPackager
        extends ISOBasePackager implements GenericPackagerParams {
    ...
    public void setGenericPackagerParams(Attributes atts) {
        ...
        if (thirdbmf != null)
            try {
                setThirdBitmapField(Integer.parseInt(thirdbmf));
            } catch (
                    ISOException e) {   // BBB throwing unchecked exception in order not to change the method's contract
